...
layout: quickstart
short: Go
description: This guide gets you started with HRPC in Go with a simple working example.
---
...
HRPC requires Go 1.6 or higher.
...
Use the following command to install HRPC.
...
Install the protoc compiler that is used to generate HRPC service code. The simplest way to do this is to download pre-compiled binaries for your platform(`protoc-<version>-<platform>.zip`) from here: [https://github.com/google/protobuf/releases](https://github.com/google/protobuf/releases)
...
HRPC services are defined in a `.proto` file, which is used to generate a corresponding `.pb.go` file. The `.pb.go` file is generated by compiling the `.proto` file using the protocol compiler: `protoc`.
...
Congratulations! You've just run a client-server application with HRPC.

### Update a HRPC service

Now let's look at how to update the application with an extra method on the
server for the client to call. Our HRPC service is defined using protocol
buffers; you can find out lots more about how to define a service in a `.proto`
file in [What is HRPC?](/docs/guides) and [HRPC Basics:
...
Next we need to update the HRPC code used by our application to use the new
...
- Read a full explanation of how HRPC works in [What is HRPC?](/docs/guides/)
  and [HRPC Concepts](/docs/guides/concepts/)
- Work through a more detailed tutorial in [HRPC Basics: Go](/docs/tutorials/basic/go/)
- Explore the HRPC Go core API in its [reference
